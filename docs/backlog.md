...
- [x] **1.17 Fix double msgpack decode in `decodeNodePutPayload`**
...
- [x] **2.1 Project setup**
  ...
  - **Cross-SDK design decisions (locked here, apply across all Epic 2 and 3 tasks):**
    - **Async model:** `Open`, `Commit`, `Close` are async (file I/O). All in-memory operations (`PutNode`, `GetNode`, `PutEdge`, etc.) are sync — they do no I/O and the async annotation would be dishonest about their cost.
    - **`uint64` timestamps:** Exposed as `number` on the public API (`CreatedAt`, `UpdatedAt`). Safe in practice — 2^53 microseconds is ~285 years from epoch. Internally, the MessagePack codec uses `bigint` where needed to handle the full uint64 range correctly.
    - **Optional `Confidence`:** `number | null` — absent means `null`, not `undefined`.
...
- [x] **2.2b Key layout**
  - Implement key builders and parsers for all five key types, using `keys_internal.go` as the reference:
    ...
    - Edge index: `ei:{toType}:{toID}:{relation}:{fromType}:{fromID}`
    - Tag: `t:{tag}:{id}`
    - Temporal: `ts:{micros}:n:{type}:{id}` and `ts:{micros}:e:…`
